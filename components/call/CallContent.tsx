...
import { View, Text, StyleSheet, Alert, useWindowDimensions, Platform } from 'react-native';
  ...
  React.useEffect(() => {
    const requestPermission = async () => {
      if (Platform.OS !== 'android') {
        console.log('ℹ️  [CallContent] Platform is', Platform.OS, '- skipping Android permission request');
        return;
      }

      if (!connectionDetails) {
        console.log('ℹ️  [CallContent] No connection details yet - skipping permission request');
        return;
      }

      try {
        console.log('📱 [CallContent] Requesting microphone permission on Android');
        
        const granted = await PermissionsAndroid.request(
          PermissionsAndroid.PERMISSIONS.RECORD_AUDIO,
          {
            title: 'Microphone Permission Required',
            message: 'This app needs access to your microphone to listen to you during calls.',
    ...
    <View style={styles.container}>
      ...
      <View style={styles.contentSection}>
        ...
        <View style={styles.livekitContainer}>
          {connectionDetails && normalizedServerUrl && connectionDetails.participantToken ? (
            <>
              {console.log('🚀 [CallContent] Connecting to LiveKit:', {
                token: typeof connectionDetails.participantToken === 'string' ? connectionDetails.participantToken.substring(0, 50) + '...' : 'NOT A STRING',
                serverUrl: normalizedServerUrl,
                connect: true,
              })}
              <LiveKitRoom
