...
export function MonthlyTrendChart({ analytics, achievements }: MonthlyTrendChartProps) {
  const monthNames = [
                  ...
                  <View style={styles.barsContainer}>
                    <View
                      style={[
                        styles.bar,
                        styles.barGreen,
                        { height: (trend.speaking_days / Math.max(maxValue, 1)) * maxHeight },
                      ]}
                    />
                    <View
                      style={[
                        styles.bar,
                        styles.barPurple,
                        { height: (trend.quiz_days / Math.max(maxValue, 1)) * maxHeight },
                      ]}
                    />
                    <View
                      style={[
                        styles.bar,
                        styles.barBlue,
                        { height: (trend.listening_days / Math.max(maxValue, 1)) * maxHeight },
                      ]}
                    />
                    <View
                      style={[
                        styles.bar,
                        styles.barViolet,
                        { height: (trend.listening_quiz_days / Math.max(maxValue, 1)) * maxHeight },
                      ]}
                    />
                  </View>
        ...
        {achievements && (
          <View style={styles.infoSection}>
            ...
            <Text style={styles.infoText}>
              XP: {Math.floor((achievements.stats.totalSpeakingTime || 0) / 60) * 2} from time +{' '}
              {(achievements.stats.fullSessions || 0) * 10} from full sessions ={' '}
