...
import type { CourseAnalytics, UserAchievements } from '@/service/CourseService';
...
type SkillType = 'Skill Development' | 'Fluency' | 'Grammar' | 'Vocabulary' | 'Speaking Time';
  ...
  const renderChart = () => {
    ...
    if (skillType === 'Skill Development' && analytics.progress) {
      ...
      const skills = [
        ...
        { label: 'Listening', value: analytics.progress.avg_listening_quiz_score },
      ...
      const maxValue = Math.max(
        ...analytics.monthlyTrends.map((t) => Math.floor(t.total_speaking_time / 60))
      );
      const maxHeight = 200;
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false}>
          <View style={styles.lineChart}>
            {analytics.monthlyTrends.map((trend, index) => {
              const height = (Math.floor(trend.total_speaking_time / 60) / Math.max(maxValue, 1)) * maxHeight;
              return (
    ...
    } else if (skillType !== 'Skill Development' && skillType !== 'Speaking Time' && analytics.skillTrends) {
      const maxValue = Math.max(...analytics.skillTrends.map((t) => t.avg_score));
      const maxHeight = 200;
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false}>
          <View style={styles.lineChart}>
            {analytics.skillTrends.map((trend, index) => {
              const height = (trend.avg_score / Math.max(maxValue, 1)) * maxHeight;
              return (
                <View key={index} style={styles.lineChartBar}>
                  <View
                    style={[
                  ...
                  />
                  <Text style={styles.lineChartLabel}>Week {trend.week_number}</Text>
                </View>
              );
            })}
          </View>
          ...
          <View style={styles.infoSection}>
            <View style={styles.infoRow}>
              ...
              <Text style={styles.infoValue}>
                {courseService.formatSpeakingTime(achievements.stats.totalSpeakingTime)}
              </Text>
            </View>
            <View style={styles.infoRow}>
              <Text style={styles.infoLabel}>Full 5-Minute Sessions:</Text>
              <Text style={styles.infoValue}>{achievements.stats.fullSessions}</Text>
            </View>
            <Text style={styles.infoText}>
              XP is calculated based on actual speaking time: 2 XP per minute + 10 XP bonus for
              each full 5-minute session.
            </Text>
          </View>
        )}
      </View>
