...
export function SmartInsight({ analytics, achievements }: SmartInsightProps) {
  const generateSmartInsight = () => {
    ...
    if (streak >= 7) {
      return `🔥 Amazing! You're on a ${streak}-day streak. Consistency is key to language learning success!`;
    }

    if (totalSpeakingTime >= 3600) {
      return `🎙️ You've practiced speaking for over an hour! That's dedication. Keep it up to unlock higher levels!`;
    }

    if (fullSessions >= 5) {
      return `🏆 You've completed ${fullSessions} full 5-minute speaking sessions. You're earning bonus XP for your commitment!`;
    }

    if (totalQuizzes >= 10) {
      return `🧠 You've completed ${totalQuizzes} quizzes! Your knowledge is growing. Try to maintain your streak!`;
    }

    if (examsPassed >= 1) {
      return `🏅 Great job passing your first exam! You've earned 50 XP. Aim for more to level up faster!`;
    }

    if (streak > 0) {
      return `🌟 You're on a ${streak}-day streak. Keep practicing daily to build momentum and earn streak bonuses!`;
    }
