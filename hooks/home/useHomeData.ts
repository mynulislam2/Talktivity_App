...
export function useHomeData(): UseHomeDataResult {
  ...
  useEffect(() => {
    if (courseLoading && !courseStatus) {
      const timer = setTimeout(() => {
        console.warn('⚠️ Course loading timeout - stopping loading indicator');
  ...
  const isLoading = ((courseLoading && !courseStatus) || subscriptionLoading) && !loadingTimeout;
  ...
  const loadCourseData = useCallback(async () => {
    console.log('🔍 loadCourseData called', {
      subscriptionLoading,
      hasSubscription: subscription?.active || subscription?.subscription?.is_free_trial,
    ...
    if (subscriptionLoading) {
      console.log('⏸️ Subscription still loading, skipping course load');
      return;
    }
    ...
    if (!hasSubscription) {
      console.log('⏸️ No subscription available, skipping course load');
      return;
    }
    ...
    if (!currentCourseStatus && initializationAttemptedRef.current) {
      console.log('⏸️ Course initialization already attempted, skipping...');
      return;
    }
    ...
    if (!currentCourseStatus && !initializationAttemptedRef.current) {
      ...
      console.log('🔄 Starting course initialization check...');
      console.log('📡 About to call loadCourseStatus API...');
      try {
        // Try to load course status first
        const result = await dispatch(loadCourseStatus()).unwrap();
        console.log('📊 loadCourseStatus result:', result ? 'Course found' : 'No course');
        
        if (!result) {
          // No course found (NO_ACTIVE_COURSE), initialize one
          console.log('🔄 No active course found, initializing...');
          console.log('📡 About to call initializeCourse API...');
          const initResult = await dispatch(initializeCourse()).unwrap();
          if (initResult) {
            console.log('✅ Course initialized successfully');
          } else {
            console.warn('⚠️ Course initialization returned no result');
          }
        } else {
          console.log('✅ Course status loaded');
        }
      } catch (error: any) {
        // Handle rate limiting (429) gracefully
        if (error?.status === 429 || error?.message?.includes('429') || error?.message?.includes('rate limit')) {
          console.warn('⚠️ Rate limited (429) - course load skipped, will retry after delay');
          ...
          console.error('❌ Error loading/initializing course:', error);
      ...
      console.log('✅ Course status already available');
    }
  ...
  useEffect(() => {
    ...
    const checkBatchProgression = async () => {
      ...
      if (currentDay !== undefined && currentWeek !== undefined) {
        if (currentDay !== lastCheckedDayRef.current || currentWeek !== lastCheckedWeekRef.current) {
      ...
      try {
        ...
        if (courseStatus) {
          ...
          console.log('🔄 Checking batch progression...');
        ...
        if (error?.status === 429 || error?.message?.includes('429')) {
          console.warn('⚠️ Rate limited (429) - batch check skipped, will retry later');
          ...
          console.warn('⚠️ Batch check error (non-critical):', error);
    ...
    if (subscription && courseStatus && isNewCourse && 
  ...
  useEffect(() => {
    ...
    if (subscriptionId && subscriptionId !== lastSubscriptionIdRef.current) {
      console.log('🔄 Subscription ID changed, resetting load flags');
      hasAttemptedLoadRef.current = false;
      initializationAttemptedRef.current = false;
    ...
    if (hasValidSubscription !== lastSubscriptionActiveRef.current && subscriptionId) {
      console.log('🔄 Subscription status changed, resetting load flags');
      hasAttemptedLoadRef.current = false;
      initializationAttemptedRef.current = false;
      lastSubscriptionActiveRef.current = hasValidSubscription;
    }
    ...
    if (hasValidSubscription && !hasAttemptedLoadRef.current) {
      ...
      lastSubscriptionActiveRef.current = hasValidSubscription;
      console.log('🔄 Subscription available, triggering course load...');
  ...
  }, [subscription?.subscription?.id, subscription?.active, subscription?.subscription?.is_free_trial, subscriptionLoading]);
