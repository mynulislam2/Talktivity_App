/**
 * useRolePlayGeneration Hook
 * 
 * Handles custom roleplay generation using AiService.
 * Now refreshes topics list from Redux after creation.
 */

import { useState, useCallback } from 'react';
import { useAppDispatch, useAppSelector } from '@/store/hooks';
import { AiService } from '@/service/AiService';
import { roleplayService } from '@/service/RoleplayService';
...
export function useRolePlayGeneration(): UseRolePlayGenerationReturn {
  ...
  const generateRolePlay = useCallback(
    async (data: RolePlayGenerationData): Promise<Topic> => {
      ...
      try {
        // Plan-based limit (Basic/FreeTrial: 5, Pro: unlimited)
        let planType = currentSubscription?.subscription?.plan_type as string | undefined;
        if (!planType) {
          try {
            const updated = await dispatch(loadSubscriptionStatus()).unwrap();
            planType = updated?.subscription?.plan_type;
          } catch {
            // If status can't be loaded, treat as Basic for safety
            planType = 'Basic';
          }
        }
        ...
        if (planType !== 'Pro') {
          ...
          if (count >= 5) {
            throw new Error('You can create up to 5 custom role-play scenarios on your current plan. Upgrade to Pro for unlimited.');
          }
        }

        const { myRole, otherRole, situation } = data;

        // Use AiService to generate roleplay prompts
        const { success, data: aiData, error } = await aiService.generateRolePlay(
          myRole,
          otherRole,
          situation
        );
        ...
        const { prompt: generatedPrompt, firstPrompt: generatedFirstPrompt } = aiData;

        // Create topic object with generated prompts
        const newCustomTopic: Topic = {
          id: `custom-ai-${Date.now()}-${Math.random().toString(36).substring(2, 9)}`,
          ...
          )}%20vs%20${encodeURIComponent(otherRole.substring(0, Math.min(otherRole.length, 5)))}`,
        ...
        // Save roleplay to backend (per-user roleplay_sessions table)
        ...
        // Map API response into Topic shape used by UI + Practice session
