...
export const logError = (error: any, context?: string) => {
  ...
  console.error(`[${timestamp}] [${errorType}]${context ? ` [${context}]` : ''}:`, errorMessage, error);
 ...
 */
export const showErrorAlert = (error: AppError | Error | string, title: string = 'Error') => {
 ...
 */
export const validateRequired = (data: Record<string, any>, fields: string[]): AppError | null => {
 ...
 */
export const validatePassword = (password: string, minLength: number = 6): AppError | null => {
...
export const withErrorHandling = async <T,>(
  ...
  try {
    ...
    const appError = error instanceof Error && (error as AppError).type
      ? (error as AppError)
