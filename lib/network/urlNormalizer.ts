...
const getDevHostIP = (): string => {
  ...
  if (devHostIP && devHostIP !== 'null' && devHostIP !== 'undefined' && String(devHostIP).trim() !== '') {
...
export function normalizeUrl(url: string): string {
  if (!url || typeof url !== 'string') {
    console.log('🔗 [URLNormalizer] Invalid URL input:', { url, type: typeof url });
    return url;
  }

  // Don't modify URLs that already have a specific IP or domain
  if (!url.includes('localhost') && !url.includes('127.0.0.1')) {
    console.log('🔗 [URLNormalizer] URL already has specific IP/domain, no normalization needed:', url);
    return url;
  }
  ...
  console.log('🔗 [URLNormalizer] Normalizing URL:', {
    ...
    devHostIP: hostIP,
    devHostIPSource: process.env.EXPO_PUBLIC_DEV_HOST_IP || Constants.expoConfig?.extra?.DEV_HOST_IP || 'default',
  });
