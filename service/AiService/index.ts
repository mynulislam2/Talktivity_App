...
class AiService {
  ...
  async generateRolePlay(
    ...
    try {
      const response = await httpService.post('/ai/generate-roleplay', {
        myRole,
        otherRole,
        situation,
      });
      ...
      return {
        success: false,
        error: error.response?.data?.error || error.message || 'Failed to generate roleplay',
      };
    }
  }
  ...
  async generateQuiz(): Promise<GenerateQuizResponse> {
    try {
      const response = await httpService.get('/ai/generate-quiz');
   ...
   */
  async generateListeningQuiz(conversation: string): Promise<GenerateQuizResponse> {
    try {
      const response = await httpService.post('/ai/generate-listening-quiz', {
        conversation,
      });
      ...
      return {
        success: false,
        error: error.response?.data?.error || error.message || 'Failed to generate listening quiz',
      };
    }
  }
}
