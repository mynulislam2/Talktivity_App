...
function normalizeOptions(raw: any, questionIndex: number): QuizOption[] {
  ...
  return arr
    .map((opt: any, idx: number) => {
      ...
      const id = String(opt?.id ?? opt?.key ?? toId(`opt-${questionIndex}`, idx));
...
function normalizeCorrectOptionIds(raw: any, options: QuizOption[]): string[] {
  const correct = raw?.correctOptionIds ?? raw?.correct_option_ids ?? raw?.correct_answer_ids ?? raw?.correctAnswers ?? raw?.correct_answers ?? raw?.answer;
  ...
  if (Array.isArray(correct)) {
    ...
    const mapped = asStrings
      .map((txt) => {
        const match = options.find((o) => o.text.trim().toLowerCase() === txt.trim().toLowerCase());
  ...
  if (typeof correct === 'string' || typeof correct === 'number') {
    ...
    const match = options.find((o) => String(o.id) === v || o.text.trim().toLowerCase() === v.trim().toLowerCase());
  ...
  if (typeof correctText === 'string') {
    const match = options.find((o) => o.text.trim().toLowerCase() === correctText.trim().toLowerCase());
...
function normalizeAIQuestions(raw: any[], type: QuizType): QuizQuestion[] {
  ...
  return list
    .map((q, idx) => {
      const questionText = String(q?.question ?? q?.prompt ?? q?.q ?? '').trim();
...
class QuizService {
   ...
   */
  async generateQuizForConversation(): Promise<{ success: boolean; data: QuizQuestion[]; error?: string }> {
    const res = await this.ai.generateQuiz();
    if (!res.success) return { success: false, data: [], error: res.error || 'Failed to generate quiz' };
    return { success: true, data: normalizeAIQuestions(res.data || [], 'speaking') };
  }

  /**
   * Preferred API: generate a listening quiz from a conversation string.
   */
  async generateListeningQuiz(conversation: string): Promise<{ success: boolean; data: QuizQuestion[]; error?: string }> {
    const res = await this.ai.generateListeningQuiz(conversation);
    if (!res.success) return { success: false, data: [], error: res.error || 'Failed to generate listening quiz' };
    return { success: true, data: normalizeAIQuestions(res.data || [], 'listening') };
  }
  ...
  async generateQuiz(data: any): Promise<any> {
    const response = await httpService.post('/quizzes/generate', data);
