...
class VocabularyService {
   ...
   */
  async getVocabularyByWeekAndDay(week?: number, day?: number): Promise<VocabularyResponse> {
    try {
      ...
      const response = await httpService.get('/vocabulary/words', {
      ...
      if (response.data?.success === false) {
        return {
          success: false,
          error: response.data.error || response.data.message || 'Failed to load vocabulary',
        };
      }
   ...
   * If week/day not provided, backend uses course's current week/day
   */
  async markVocabularyAsCompleted(week?: number, day?: number): Promise<MarkCompleteResponse> {
    try {
      ...
      const response = await httpService.post('/vocabulary/complete', payload);
      ...
      return {
        success: false,
        error: response.data?.error || response.data?.message || 'Failed to mark vocabulary as completed',
      };
    } catch (error: any) {
