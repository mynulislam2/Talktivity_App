...
export type OnboardingRequiredRoute =
  (typeof ONBOARDING_REQUIRED_ROUTES)[number];
export type ConversationExperienceRequiredRoute =
  (typeof CONVERSATION_EXPERIENCE_REQUIRED_ROUTES)[number];
...
export function requiresOnboarding(pathname: string): boolean {
  return ONBOARDING_REQUIRED_ROUTES.includes(
    pathname as OnboardingRequiredRoute
  );
}
...
export function requiresConversationExperience(pathname: string): boolean {
  return CONVERSATION_EXPERIENCE_REQUIRED_ROUTES.includes(
    pathname as ConversationExperienceRequiredRoute
  );
}
