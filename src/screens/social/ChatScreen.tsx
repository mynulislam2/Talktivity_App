...
const ChatScreen: React.FC<ChatScreenProps> = ({ navigation, route }) => {
  ...
  const [selectedDmId, setSelectedDmId] = useState<number | null>(
    route.params?.dmId || null
  );
  ...
  const messagesSelector = useMemo(() => {
    if (!selectedDmId) {
      return createSelector([(state: any) => state], () => []);
    }
    const baseSelector = selectDMMessages(selectedDmId);
    return createSelector([(state: any) => state], (state) => {
      try {
  ...
  const messagesLoadingSelector = useMemo(() => {
    if (!selectedDmId) {
      return createSelector([(state: any) => state], () => false);
    }
  ...
  const typingUserIdSelector = useMemo(() => {
    if (!selectedDmId) {
      return createSelector([(state: any) => state], () => null);
    }
  ...
  useEffect(() => {
    if (selectedDM) {
      navigation.setOptions({
        title: (selectedDM as any).other_user?.name || 'Chat',
      });
    }
              ...
              >
                <View style={styles.contactAvatar}>
                  <Ionicons
                    ...
                    color={colors.primary}
                  />
                  {(dm as any).other_user?.is_online && (
                ...
                <View style={styles.contactContent}>
                  <Text style={styles.contactName}>
                    {(dm as any).other_user?.name || 'Unknown'}
                  </Text>
                  <Text style={styles.lastMessage} numberOfLines={1}>
                    {(dm as any).last_message_content || 'No messages yet'}
                  </Text>
                </View>
      ...
      <View style={styles.chatHeader}>
        ...
        <View style={styles.chatHeaderContent}>
          <Text style={styles.chatHeaderTitle}>
            {(selectedDM as any)?.other_user?.name || 'Chat'}
          </Text>
          <View style={styles.statusContainer}>
            {(selectedDM as any)?.other_user?.is_online && (
              <>
            ...
            {typingUserId && (
              <View style={styles.typingIndicator}>
                <Text style={styles.typingText}>
                  {(selectedDM as any)?.other_user?.name} is typing...
                </Text>
              </View>
            )}
          </>
