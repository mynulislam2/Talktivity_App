...
class TopicService {
  ...
  async getTopics(): Promise<TopicsResponse> {
    try {
      const response = await httpService.get(API_URLS.TOPIC.LIST);
  ...
  async getTopicById(topicId: string): Promise<Topic> {
    try {
      const response = await httpService.get(API_URLS.TOPIC.DETAIL(topicId));
      ...
      throw new Error(
        error.response?.data?.error || error.message || 'Failed to load topic'
      );
    }
  }
  ...
  async createTopic(
    ...
    try {
      const response = await httpService.post(API_URLS.TOPIC.LIST, {
        category,
        topic,
      });
      ...
      throw new Error(
        error.response?.data?.error || error.message || 'Failed to create topic'
      );
    }
  }
}
