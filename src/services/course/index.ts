...
class CourseService {
  ...
  async initializeCourse(): Promise<any> {
    const response = await httpService.post(API_URLS.COURSE.INITIALIZE, {});
    return response.data;
  }

  // 2. Get active course (no batch trigger)
  async getCourseStatus(): Promise<CourseStatus> {
    const response = await httpService.get(API_URLS.COURSE.GET_ACTIVE);
  ...
  async checkAndCreateNextBatch(): Promise<any> {
    const response = await httpService.post(
      API_URLS.COURSE.CHECK_NEXT_BATCH,
      {}
    );
    return response.data;
  }
  ...
  async getFullCourseTimeline(date?: string): Promise<{
    ...
    const response = await httpService.get(API_URLS.COURSE.TIMELINE, {
  ...
  async getCourseAnalytics(): Promise<CourseAnalytics> {
    try {
      const response = await httpService.get(API_URLS.PROGRESS.OVERVIEW);
      const data = response.data;
  ...
  async getMonthlyReport(month: number, year: number): Promise<MonthlyReport> {
    ...
    try {
      const response = await httpService.get(
        `${API_URLS.REPORT.MONTHLY}?month=${month}&year=${year}`
      );
      const data = response.data;
  ...
  async getProgressPageData(): Promise<{
