...
class LeaderboardService {
  ...
  async getWeeklyLeaderboard(): Promise<LeaderboardResponse> {
    try {
      const response = await httpService.get(API_URLS.LEADERBOARD, {
        params: { period: 'week' },
      });
      ...
      throw new Error(
        response.data?.error || 'Failed to fetch weekly leaderboard'
      );
    } catch (error: any) {
  ...
  async getOverallLeaderboard(): Promise<LeaderboardResponse> {
    try {
      ...
      throw new Error(
        response.data?.error || 'Failed to fetch overall leaderboard'
      );
    } catch (error: any) {
