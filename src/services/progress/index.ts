...
export type DailyProgressUpdates = Partial<DailyProgressRow> & {
  date?: string;
};
...
class ProgressService {
  ...
  async getDailyProgress(date?: string): Promise<DailyProgressResponse> {
    try {
      const response = await httpService.get(API_URLS.PROGRESS.DAILY, {
  ...
  async updateDailyProgress(
    updates: DailyProgressUpdates
  ): Promise<DailyProgressResponse> {
    try {
      const response = await httpService.post(
        API_URLS.PROGRESS.DAILY,
        updates || {}
      );
