...
export const chatSlice = createSlice({
  ...
  reducers: {
    dmMessageReceived: (
      state,
      action: PayloadAction<{ dmId: number; message: any }>
    ) => {
    ...
    groupMessageReceived: (
      state,
      action: PayloadAction<{ groupId: number; message: any }>
    ) => {
    ...
    dmTypingUpdated: (
      state,
      action: PayloadAction<{ dmId: number; userId: number; typing: boolean }>
    ) => {
...
const selectDMRealtimeMessagesMap = (state: RootState) =>
  state.chat.dmRealtimeMessages;
const selectGroupRealtimeMessagesMap = (state: RootState) =>
  state.chat.groupRealtimeMessages;
const selectDMTypingUserMap = (state: RootState) => state.chat.dmTypingUser;
const selectGroupTypingUsersMap = (state: RootState) =>
  state.chat.groupTypingUsers;
