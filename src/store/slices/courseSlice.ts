...
import { courseService, type CourseStatus } from '@/services/course';
  ...
  async (_, { rejectWithValue }) => {
    try {
      ...
      if (error?.status === 429 || error?.response?.status === 429) {
        console.warn('âš ï¸ Rate limited (429) - course status load skipped');
...
export const initializeCourse = createAsyncThunk(
  ...
  async (_, { rejectWithValue }) => {
    try {
      ...
      if (error?.status === 429 || error?.response?.status === 429) {
        console.warn(
          'âš ï¸ Rate limited (429) - course initialization skipped'
        );
...
export const checkAndCreateNextBatch = createAsyncThunk(
  ...
  async (_, { rejectWithValue }) => {
    try {
      ...
      if (error?.status === 429 || error?.response?.status === 429) {
        console.warn('âš ï¸ Rate limited (429) - batch check skipped');
      ...
      return rejectWithValue(
        error.message || 'Failed to check batch progression'
      );
    }
  }
);
 ...
 * Selectors
 */
export const selectCourseStatus = (state: { course: CourseState }) =>
  state.course.courseStatus;
export const selectCourseLoading = (state: { course: CourseState }) =>
  state.course.loading || state.course.initializing;
export const selectCourseError = (state: { course: CourseState }) =>
  state.course.error;
export const selectIsInitializing = (state: { course: CourseState }) =>
  state.course.initializing;
export const selectIsCheckingBatch = (state: { course: CourseState }) =>
  state.course.checkingBatch;
