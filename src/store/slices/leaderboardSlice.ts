  ...
  async (_, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(
        response.error || 'Failed to load weekly leaderboard'
      );
    } catch (error: any) {
      return rejectWithValue(
        error.message || 'Failed to load weekly leaderboard'
      );
    }
  }
);
...
export const loadOverallLeaderboard = createAsyncThunk(
  ...
  async (_, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(
        response.error || 'Failed to load overall leaderboard'
      );
    } catch (error: any) {
      return rejectWithValue(
        error.message || 'Failed to load overall leaderboard'
      );
    }
  }
);
...
export const { setLeaderboardType, clearError, resetLeaderboard } =
  leaderboardSlice.actions;

/**
 * Selectors
 */
export const selectWeeklyLeaderboard = (state: {
  leaderboard: LeaderboardState;
}) => state.leaderboard.weeklyLeaderboard;
export const selectOverallLeaderboard = (state: {
  leaderboard: LeaderboardState;
...
export const selectUserPosition =
  (type: LeaderboardType) => (state: { leaderboard: LeaderboardState }) =>
    state.leaderboard.userPosition[type];
export const selectLeaderboardLoading = (state: {
  leaderboard: LeaderboardState;
}) =>
  state.leaderboard.loading ||
  state.leaderboard.weeklyLoading ||
  state.leaderboard.overallLoading;
...
export const selectCurrentUserPosition = (state: {
  leaderboard: LeaderboardState;
}) => {
  const currentType = state.leaderboard.currentType;
