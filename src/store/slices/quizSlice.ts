...
export const generateListeningQuizQuestions = createAsyncThunk(
  ...
  async (conversation: string, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(
        result.error || 'Failed to generate listening quiz'
      );
    } catch (error: any) {
      return rejectWithValue(
        error?.response?.data?.error ||
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(generateListeningQuizQuestions.rejected, (state, action) => {
        state.loading = false;
        state.error =
          (action.payload as string) || 'Failed to generate listening quiz';
      });
  },
});

export const { setLastQuiz, setLastListeningQuiz, resetQuiz, clearError } =
  quizSlice.actions;
export default quizSlice.reducer;

// Selectors
export const selectLastQuiz = (state: RootState) => state.quiz.lastQuiz;
export const selectLastListeningQuiz = (state: RootState) =>
  state.quiz.lastListeningQuiz;
export const selectQuizQuestions = (state: RootState) => state.quiz.questions;
export const selectListeningQuizQuestions = (state: RootState) =>
  state.quiz.listeningQuestions;
