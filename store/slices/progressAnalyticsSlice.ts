...
import { courseService, type CourseAnalytics, type UserAchievements } from '@/service/CourseService';
...
export const loadProgressAnalytics = createAsyncThunk(
  ...
  async (_, { rejectWithValue }) => {
    try {
      ...
      if (error?.message?.includes('404') || error?.message?.includes('No active course')) {
        return null;
      }
      return rejectWithValue(error.message || 'Failed to load progress analytics');
    }
  }
);
...
export const refreshProgressAnalytics = createAsyncThunk(
  ...
  async (_, { dispatch, rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(error.message || 'Failed to refresh progress analytics');
    }
  }
);
  ...
  extraReducers: (builder) => {
    ...
    builder
      ...
      .addCase(loadProgressAnalytics.rejected, (state, action) => {
        ...
        if (action.payload && !String(action.payload).includes('404') && !String(action.payload).includes('No active course')) {
...
export const { clearError, resetProgressAnalytics } = progressAnalyticsSlice.actions;

/**
 * Selectors
 */
export const selectProgressAnalytics = (state: { progressAnalytics: ProgressAnalyticsState }) => 
  state.progressAnalytics.analytics;
export const selectProgressAchievements = (state: { progressAnalytics: ProgressAnalyticsState }) => 
  state.progressAnalytics.achievements;
export const selectProgressLoading = (state: { progressAnalytics: ProgressAnalyticsState }) => 
  state.progressAnalytics.loading;
export const selectProgressError = (state: { progressAnalytics: ProgressAnalyticsState }) => 
