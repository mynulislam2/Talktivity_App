 ...
 * Subscription Slice
 * 
 * Manages subscription state globally using Redux Toolkit.
 * Handles subscription plans, status, and free trial activation.
 */

import { createSlice, createAsyncThunk, PayloadAction } from '@reduxjs/toolkit';
import { subscriptionService, SubscriptionPlan, SubscriptionStatus, FreeTrialResponse } from '@/service/SubscriptionService';
...
// Async thunk for loading subscription status
  ...
  async (_, { rejectWithValue }) => {
    try {
      ...
      if (response.success && response.data) {
        ...
        return rejectWithValue(response.error || 'Failed to load subscription status');
      }
    } catch (error: any) {
      return rejectWithValue(error.message || 'Failed to load subscription status');
    }
  }
);

// Async thunk for starting free trial
...
const subscriptionSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(loadPlans.fulfilled, (state, action: PayloadAction<SubscriptionPlan[]>) => {
        state.loading = false;
      ...
      // Handle loadSubscriptionStatus
      .addCase(loadSubscriptionStatus.pending, (state) => {
        state.loadingStatus = true;
        state.errorStatus = null;
      })
      .addCase(loadSubscriptionStatus.fulfilled, (state, action: PayloadAction<SubscriptionStatus>) => {
        state.loadingStatus = false;
      ...
      .addCase(loadSubscriptionStatus.rejected, (state, action) => {
        state.loadingStatus = false;
        state.errorStatus = action.payload as string || 'Failed to load subscription status';
      })
      // Handle startFreeTrial
      .addCase(startFreeTrial.pending, (state) => {
        state.startingTrial = true;
        state.error = null;
      })
      .addCase(startFreeTrial.fulfilled, (state, action: PayloadAction<FreeTrialResponse['data']>) => {
        state.startingTrial = false;
        // Update current subscription with new trial data
      ...
      .addCase(startFreeTrial.rejected, (state, action) => {
        state.startingTrial = false;
        state.error = action.payload as string || 'Failed to start free trial';
      });
  },
});
...
export const selectStartingTrial = (state: { subscription: SubscriptionState }) => state.subscription.startingTrial;
export const selectSubscriptionStatusLoading = (state: { subscription: SubscriptionState }) => state.subscription.loadingStatus;
export const selectSubscriptionStatusError = (state: { subscription: SubscriptionState }) => state.subscription.errorStatus;
export const selectCanStartFreeTrial = (state: { subscription: SubscriptionState }) => 
